...
$out_file = mkpath(getcwd().DS.'app'.DS.'controllers').DS.$name.EXT;
...
if (is_file($out_file)) {
  ...
  $route_file = getcwd().DS.'app'.DS.'routes'.EXT;
  ...
  if (cli::flag('helper')) {
    ...
    write(mkpath(getcwd().DS.'app'.DS.'helpers').DS.$name.EXT, "<?php\n");
  }
  ...
  if (cli::flag('view')) {
    ...
    write(mkpath(getcwd().DS.'app'.DS.'views'.DS.$name).DS.'index'.EXT, $text);
  }
}
