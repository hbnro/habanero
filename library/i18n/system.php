...
function ln($input)
{
  ...
  else
  {
    ...
    $input    = call_user_func_array($callback, $args);
  }
 ...
 * @return string
 */
function pluralize($number, $string, array $params = array())
{
...
function translate($string, $default = '', array $params = array())
{
  ...
  if (is_array($params['default']))
  {
    foreach ($params['default'] as $one)
    {
      ...
      {
        $test = translate($one, array('scope' => $params['scope']));
 ...
 * @return array
 */
function load_locale(array $set = array(), $scope = '')
{
  static $tree = array();
